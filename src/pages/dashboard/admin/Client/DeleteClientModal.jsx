...
import { X, AlertTriangle, Trash2, Shield, User, Users } from "lucide-react";
...
const DeleteClientModal = ({ isOpen, onClose, onConfirm, client }) => {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      console.error(t("error_deleting_client"), error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4"
      ...
      <div className="relative bg-gray-900/95 backdrop-blur-xl border-2 border-red-400/30 rounded-3xl w-full max-w-md overflow-x-auto max-h-[95vh]"
        ...
        <div className="relative z-10 p-8">
          ...
          <div className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-6 mb-8">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="text-gray-400 text-sm">{t("deleting_client")}</p>
                <p className="text-white font-semibold">
                  {client?.name || t("deleteClientModal.noName")}
                </p>
                {client?.contact_person && (
                  <p className="text-red-400 text-sm">
                    {t("contact_colon")} {client.contact_person}
                  </p>
                )}
                {client?.phones && (
                  <p className="text-gray-400 text-xs">
                    {t("phone_colon")} {client.phones}
                  </p>
                )}
              </div>
            </div>
              ...
              <div className="relative flex items-center justify-center gap-2">
                {isLoading ? (
                  ...
                  <>
                    ...
                    <span>{t("deleteClientModal.delete")}</span>
                  </>
                )}
              </div>
            </button>
