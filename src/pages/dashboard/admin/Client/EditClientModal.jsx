...
import { X, Edit3, User, Contact, Phone, MapPin, Hash, DollarSign, Users, Save } from "lucide-react";
...
const EditClientModal = ({ isOpen, onClose, client, onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(t("error_updating_client"), error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"
      ...
      <div className="relative bg-gray-900/95 backdrop-blur-xl border-2 border-cyan-400/30 rounded-3xl w-full max-w-2xl overflow-hidden"
        ...
        <div className="px-8 py-6 relative z-10">
          ...
          {client && (
            <div className="bg-gray-800/30 border border-gray-700/50 rounded-2xl p-4 mb-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-gray-400 text-sm">{t("editClientModal.title")}</p>
                  <p className="text-white font-semibold">{client.name}</p>
                  {client.contact_person && (
                    <p className="text-cyan-400 text-xs">{t("contact_colon")} {client.contact_person}</p>
                  )}
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-3">
                ...
                {t("editClientModal.name")} <span className="text-cyan-400">*</span>
              </label>
              <input
                name="name"
                ...
                style={{ 
                  boxShadow: form.name ? '0 0 15px rgba(6, 182, 212, 0.2)' : 'none'
                }}
              />
            </div>
            ...
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-3">
                <Contact className="inline w-4 h-4 mr-2 text-emerald-400" 
                         style={{ filter: 'drop-shadow(0 0 8px #10b981)' }} />
                {t("editClientModal.contactPerson")}
              </label>
              <input
                name="contact_person"
                ...
                style={{ 
                  boxShadow: form.contact_person ? '0 0 15px rgba(16, 185, 129, 0.2)' : 'none'
                }}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                style={{ 
                  boxShadow: form.phones ? '0 0 15px rgba(168, 85, 247, 0.2)' : 'none'
                }}
              />
            </div>
            ...
            <div>
              <label className="block text-gray-400 text-sm font-medium mb-3">
                <Hash className="inline w-4 h-4 mr-2 text-amber-400" 
                      style={{ filter: 'drop-shadow(0 0 8px #f59e0b)' }} />
                {t("editClientModal.username")} <span className="text-cyan-400">*</span>
              </label>
              <input
                name="username"
                ...
                style={{ 
                  boxShadow: form.username ? '0 0 15px rgba(245, 158, 11, 0.2)' : 'none'
                }}
              />
            </div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full mt-8 relative bg-gradient-to-r from-cyan-500 to-emerald-500 text-white font-bold px-8 py-4 rounded-2xl transition-all duration-300 hover:from-cyan-600 hover:to-emerald-600 hover:scale-[1.02] hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed overflow-hidden group"
            style={{ 
            ...
            <div className="relative flex items-center justify-center gap-3">
              {isLoading ? (
                ...
                <>
                  ...
                  <span>{t("editClientModal.save")}</span>
                </>
              )}
            </div>
          </button>
